...
def bindings_from_specs(specs: LaserSpecs) -> ControlBindings:
    ...
    return ControlBindings(
        ...
        scan_speed=_numeric(
            ...
            value=int(round(specs.scan_speed)) if specs.scan_speed is not None else None,
            minimum=int(specs.scan_speed_min) if specs.scan_speed_min is not None else None,
            maximum=int(specs.scan_speed_max) if specs.scan_speed_max is not None else None,
            step=1.0,
        ),
        ...
        trigger_polarity=_select(
            enabled=specs.trigger_polarity is not None,
            value=int(specs.trigger_polarity)
            if specs.trigger_polarity is not None
            else int(TriggerPolarity.ACTIVE_HIGH),
        ),
